...
def check():
    ...
    if len(res)>=1:
        ...
        root1.destroy()
        time.sleep(1)
        notification.notify(title='Woah!',message='Welcome Ansh',app_icon="C:\Python\icon1.ico",timeout=10,toast=None)
        start()
    else:
        engine.say("incorrect User id or password")
        engine.runAndWait()
...
def login():
    ...
    #time.sleep(1)
    #engine.say("Enter User id and password")
    #engine.runAndWait()
...
def check1():
    ...
    if x==1:
        ...
        db1.commit()
        engine.say("Member Added Sucessfully")
        engine.runAndWait()
        ...
        if len(res)>=1:
            if d1==1:
                ...
                if len(res)==1:
                    ...
                    c3.execute(q,val)
                    db1.commit()
                    engine.say("Updated Successfuly")
                    engine.runAndWait()
...
def Quit1():
    engine.say("Thank You..bye bye")
    engine.runAndWait()
    notification.notify(title='Thank You',message='I Hope You Like It',app_icon="C:\Python\icon1.ico",timeout=10,toast=None)
    root.destroy()
